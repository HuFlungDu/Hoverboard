...
# Pretend to import the entire standard library so it is all available from py2exe compiled exe, for plugins
if False:
    import AutoComplete
...
def create_menu_item(mainmenu,menu, label, func, bitmap=None):
...
class TaskBarIcon(wx.TaskBarIcon):
    ...
    def CreatePopupMenu(self):
        ...
        create_menu_item(menu,menu, "Settings", self.on_settings)
        create_menu_item(menu,menu, label, self.on_pause)
        if not hoverboard.config.auto_push:
            create_menu_item(menu,menu, "Push clipboard now", functools.partial(self.on_push,device=None))
        if not hoverboard.config.auto_pull_global:
            create_menu_item(menu,menu, "Pull clipboard now", functools.partial(self.on_pull,device=None))
        if not hoverboard.config.auto_pull_device:
            create_menu_item(menu,menu, "Pull device clipboard now", functools.partial(self.on_pull,device=hoverboard.settings.device_name))
        if len(hoverboard.devices):
            ...
            for device in hoverboard.devices:
                ...
                item = create_menu_item(menu,submenu, device.name, functools.partial(self.on_push,device=device), bitmap)
            menu.AppendMenu(wx.ID_ANY,"Push to device...",submenu)
        menu.AppendSeparator()
        create_menu_item(menu,menu, 'Exit Hoverboard', self.on_exit)
        return menu
...
def main(argv=None):
    ...
    hoverboard.init(args,settings,os.path.join(SCRIPT_ROOT,"plugins"))
    #app = wx.App()
